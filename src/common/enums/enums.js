...
class I {
    constructor (item) {
        ...
        this.value = typeof item.value === 'number' ? item.value : null;
    };
...
class Enums {
    ...
    getValue (text) {
        let items = this.items;
        let buffer = items.filter(function (item) { return item.text === text; });

        return buffer && buffer.length > 0 ? buffer[0].value : null;
    }

    getText (value) {
        let items = this.items;
        let buffer = items.filter(function (item) { return item.value === value; });

        return buffer && buffer.length > 0 ? buffer[0].text : null;
    }
